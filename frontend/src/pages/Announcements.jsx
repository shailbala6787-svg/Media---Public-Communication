...
const audienceMap = { all: 'badge-info', staff: 'badge-purple', public: 'badge-teal', media: 'badge-neutral' };
...
export default function Announcements() {
  ...
  const fetch = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get('/api/announcements', { params: { page, limit: 10, search, status, priority } });
  ...
  const handleDelete = async () => {
    setDelLoading(true);
    try { await axios.delete(`/api/announcements/${deleteId}`); setDeleteId(null); fetch(); }
    catch (e) { console.error(e); } finally { setDelLoading(false); }
  };

  const columns = [
    { key: 'title', label: t('common.title'), render: (v, r) => <span className="td-title">{r.pinned && '📌 '} {v}</span> },
    { key: 'priority', label: t('common.priority'), render: (v) => <span className={`badge ${priorityColors[v]}`}><span className={`priority-dot priority-${v}`} />{t(`announcements.priority.${v}`)}</span> },
    ...
    ...(user?.role !== 'viewer' ? [{
      ...
      render: (v) => (
        <div style={{ display: 'flex', gap: 6 }}>
          <button className="btn btn-secondary btn-sm btn-icon" onClick={() => navigate(`/announcements/edit/${v}`)}><MdEdit size={15} /></button>
  ...
  return (
    <div className="animate-fade">
      ...
      <ConfirmModal isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete} loading={delLoading} title="Delete Announcement" message="Are you sure you want to delete this announcement?" />
    </div>
  );
}
